...
location_coordinate_scaled = location_coordinate./max(location_coordinate,[],2);
w = zeros(1,N_edges);
for i = 1:N_edges
   w(i) = norm(location_coordinate_scaled(:,s(i)) - location_coordinate_scaled(:,t(i))); 
end
figure()
location_topology = graph(s,t,w);
NodeColor = [repmat([1 0 0], N_locations_per_type(location_CRDC), 1);
             repmat([0 0 1], N_IO_location, 1)];
plot(location_topology,'XData',location_coordinate_scaled(1,:),'YData',location_coordinate_scaled(2,:),'NodeColor',NodeColor,'EdgeColor',[0 0 0],'EdgeLabel',location_topology.Edges.Weight)